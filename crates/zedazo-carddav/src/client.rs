...
/// Resultado de un pull de solo lectura.
#[derive(Debug, Clone)]
...
/// Cliente CardDAV de solo lectura.
...
impl CardDavClient {
    ...
    pub fn pull(&self, addressbook: Option<&Url>) -> CardDavResult<PullResult> {
        ...
        let cards = self.download_cards(&selected.url)?;
        Ok(PullResult {
    ...
    fn send_follow(
        ...
        let start_host = url.host_str().map(str::to_string);
        let mut hops = 0usize;
        loop {
            ...
            if cross_host && !allow_cross_host {
                return Err(CardDavError::Protocol(format!(
                    "redirección fuera de origen bloqueada: {url}"
                )));
            }
            let use_auth = send_auth && !cross_host;
            let resp = self.send_with_retry(&method, &url, depth, body.as_deref(), use_auth)?;
            ...
            if hops > MAX_REDIRECTS {
                return Err(CardDavError::Protocol(format!(
                    "demasiadas redirecciones desde {url}"
                )));
            }
            let Some(loc) = resp.headers().get(reqwest::header::LOCATION) else {
                return Err(CardDavError::Protocol(format!(
                    "redirección sin Location desde {url}"
                )));
            };
            let loc = loc
                .to_str()
    ...
    fn send_with_retry(
        ...
        for attempt in 0..MAX_RETRIES {
            let resp = self.dispatch(method, url, depth, body, send_auth)?;
            let status = resp.status();
