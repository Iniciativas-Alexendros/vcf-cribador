...
| **O4** Clasificar taxonomía N1/N2/N3 | `domain::classification`, `domain::rules`, `infrastructure::config` | `test_juzgado_matched`, `test_domain_matched`, `test_multiple_categories`, `test_crypto_matched`, `test_no_match_defaults`, `test_all_rules_compile` | `test_cribar_proton_real`, `test_config_custom_rules` | ⚠️ N3 parcial |
...
| **O10** Paridad CLI↔GUI | `docs/gui`, `zedazo-api` | (equivalencia post-v0.5) | (pendiente Fase 5) | ⚠️ Hito v0.5.0 |
...
| **I4** Salida VCF 4.0 canónica | `infrastructure::writer`, `domain::verification` | `test_write_vcf_4_0_version`, `test_folding_75_octets`, `test_folding_no_multibyte_split` | `test_pipeline_completo` | ✅ Integrada (`verify_post`) |
| **I5** Compatibilidad entrada 3.0 | `infrastructure::v3_compat`, `domain::verification` | `test_parse_single_vcard_3_0`, `test_into_contact_drops_agent`, `test_v3_props_not_emitted` | `test_cribar_google_v3`, `test_cribar_apple_v3` | ✅ Integrada (`verify_post`) |
| **I6** Auditabilidad DecisionTrace | `domain::screening`, `domain::audit`, `domain::verification` | `test_decision_trace_immutable`, `test_all_conserved_have_n1` | `test_audit_tsv_completeness` | ✅ Integrada (`verify_post`) |
| **I7** No destrucción VCF original | `domain::verification`, `application::cribar` | `test_verify_input_equals_output` | (manual) | ✅ Integrada en pipeline (`verify`) |
EOF
...
SRC_ROOT="${REPO_ROOT}/crates/zedazo-core/src"

# Domain modules
for module in contact screening classification identity normalization rules audit verification; do
    count=$(grep -c "#\[test\]" "${SRC_ROOT}/domain/${module}.rs" 2>/dev/null || echo 0)
...
for module in parser writer encoding v3_compat source config csv_writer json_writer tsv_writer; do
    count=$(grep -c "#\[test\]" "${SRC_ROOT}/infrastructure/${module}.rs" 2>/dev/null || echo 0)
    count=$(echo "$count" | tr -d '\n')
    echo "| \`infrastructure::${module}\` | ${count} | (ver tabla arriba) |" >> "${OUTPUT_FILE}"
done

# Application modules
for module in cribar audit stats export process; do
    count=$(grep -c "#\[test\]" "${SRC_ROOT}/application/${module}.rs" 2>/dev/null || echo 0)
    count=$(echo "$count" | tr -d '\n')
    echo "| \`application::${module}\` | ${count} | (ver tabla arriba) |" >> "${OUTPUT_FILE}"
done

# Integration tests
int_count=$(grep -c "#\[test\]" "${REPO_ROOT}/crates/zedazo-core/tests/integration_test.rs" 2>/dev/null || echo 0)
